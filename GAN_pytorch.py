...
def show_result(G_net, z_, num_epoch, show=True, save=True, path='result.png'):
    ...
    subplots_figure = plt.subplots(5, 5, figsize=(5, 5))
    fig = subplots_figure[0]
    axs = subplots_figure[1]
    fig.suptitle('No. of Epoch #{}'.format(num_epoch))

    all_axs = axs.flat
    nn_weights_img = G_net(z_).cpu().detach().numpy()

    for i,ax in enumerate(all_axs):
        ax.imshow(nn_weights_img[i].reshape((28, 28)), cmap='gray')
